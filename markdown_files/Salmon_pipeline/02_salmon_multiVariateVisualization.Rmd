...
```{r}
gc_majorLoading <- gc[row.names(gc) == "LOC111112914",]
majorLoading_counts <- as.data.frame(cbind(Trt=model$Treatment,Time=model$Time,SFV=model$SFV,Pop=model$Pop,t(gc_majorLoading)))

ggplot(majorLoading_counts,aes(x=as.factor(SFV),y=LOC111112914)) + geom_boxplot() + 
  labs(x= c("SFV"))
```
