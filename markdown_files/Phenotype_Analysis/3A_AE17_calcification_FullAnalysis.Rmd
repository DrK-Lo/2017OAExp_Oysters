...
cal<-read.csv("/home/downeyam/Github/2017OAExp_Oysters/input_files/Phenotype/EPF/AE17_BouyantWeight_Final_20191115 .csv")
# Final version of sample data
samp<-read.csv("/home/downeyam/Github/2017OAExp_Oysters/input_files/Phenotype/AE17_Exp_2017_CollectionInfo_20190227.csv")
# Final EPF fluid data 
pheno <- readRDS("/home/downeyam/Github/2017OAExp_Oysters/input_files/Phenotype/finalFullPhenotypeData.RData")
pheno_red <- subset(pheno,select=c(1,30,37))
...
# Remove the extra oysters used for the calcein experiment
cal_red2 <- cal_red[cal_red$sample_date < 20170825,]
cal_red3 <-  cal_red2[cal_red2$sample_date < 20170824,]
```
...
ggplot(cal_red3,aes(x=as.factor(pCO2),y=bw2_bw3_daily,group=as.factor(pCO2))) + geom_boxplot()
# Table of points
table(cal_red3$pCO2)
# ANOVA and post hoc
out.aov <- aov(cal_red3$bw2_bw3_daily~as.factor(cal_red3$pCO2))
summary(out.aov)
TukeyHSD(out.aov)
# box plot (pco2 as as continuous variable)
ggplot(cal_red3,aes(x=pCO2,y=bw2_bw3_daily,group=as.factor(pCO2))) + geom_boxplot()

## Scaling by oyster size
# box plot (pco2 as as continuous variable)
ggplot(cal_red3,aes(x=pCO2,y=bw2_bw3_daily/dry2_final,group=as.factor(pCO2))) + geom_boxplot()
# ANOVA and post hoc
out.aov <- aov(bw2_bw3_daily/dry2_final~as.factor(pCO2),data=cal_red3)
summary(out.aov)
TukeyHSD(out.aov)
# Linear model
out.lm <- lm(bw2_bw3_daily/dry2_final~pCO2_calc,data=cal_red3)
summary(out.lm)
ggplot(cal_red3,aes(x=pCO2_calc,y=bw2_bw3_daily/dry2_final,colour=as.factor(pCO2))) + geom_point()
```
...
```{r}
# Table
table(cal_red3$sample_date,cal_red3$pCO2)
# Total number of samples
nrow(cal_red3)
```
...
**ANOVA**
```{r}
cal_red3$calcification <- cal_red3$bw2_bw3_daily/cal_red3$dry2_final

# Full model- All points
cal_full <- lmer(calcification~as.factor(pCO2) + (1|PopOrigin) + (1|shelf/tank),data=cal_red3)
...
cal_full_2 <- lmer(calcification~as.factor(pCO2) + (1|tank:shelf),data=cal_red3)
ranova(cal_full_2)
# Random effect still not significant but i will keep since it's close 
summary(cal_full_2)

# Fixed effects only
cal_analysis_fixed <- lm(calcification~as.factor(pCO2),data=cal_red3)
...
cal_lm_full <- lmer(calcification~pCO2_calc + (1|PopOrigin) + (1|shelf/tank),data=cal_red3)
ranova(cal_lm_full)
#Can remove are not significant
cal_lm_Rtank <- lmer(calcification~pCO2_calc + (1|tank:shelf),data=cal_red3)
# 
...
cal_simple <- lm(calcification*100~pCO2_calc,data=cal_red3)
...
```{r}
cal_epf <- inner_join(cal_red3,pheno_red)
...
mean_cal <- aggregate(calcification*100~pCO2,cal_red3,FUN=mean)
se_cal <- aggregate(calcification*100~pCO2,cal_red3,FUN=se)
se_pco2 <- aggregate(pCO2_calc~pCO2,cal_red3,FUN=sd)
...
# ggplot version
out <- summary(lm(calcification*100~pCO2,data=cal_red3))
