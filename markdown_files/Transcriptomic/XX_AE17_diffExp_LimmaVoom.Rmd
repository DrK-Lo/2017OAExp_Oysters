...
geneC_full_cpm <- cpm(geneC_full)
geneC_sub <- geneC_full_cpm[!is.na(match(rownames(geneC_full_cpm),tl$Location)),]

total <- data.frame(type="Total",log10_Expression=log10(rowSums((geneC_full_cpm))))
target <- data.frame(type="Target",log10_Expression=log10(rowSums((geneC_sub))))
full_vals <- rbind(total,target)
  
hist(total$log10_Expression,col=adjustcolor("red",alpha.f = 0.2),breaks=100,freq = FALSE,
     xlim=c(-2,10),xlab="Log10 Expression",main="Gene Expression",)
par(new=TRUE)
hist(target$log10_Expression,col=adjustcolor("blue",alpha.f = 0.2),breaks=50,freq=FALSE,
     xlim=c(-2,10),axes = FALSE,xlab="",ylab="",main="")
legend(x=6.1,y=0.4,legend=c("All Genes","Biomineralization Genes"),pch=15,
