...
```{r}
test <- t(scale(t(gc_reduce)))
test2 <- test + abs(min(test))
hist(rowMeans(test))
hist(rowMeans(test2))
mean(test2[1,])
sd(gc_reduce[,1])
...
(out_gc <- adonis(t(test2)~Treatment*Time+Pop+Lane,data=model,permutations = 5000))
(out_gc <- adonis((test2)~Treatment*Time+Pop+Lane,data=model,permutations = 5000))
?adonis()
...
```{r}
early_time_counts <- test2[,model$Day == 9]
...
Day 80 samples**  
```{r}
late_time_counts <- test2[,model$Day == 80]
