...
geneC_new_t <- voom(geneC_new)
geneC_a2 <- geneC_new_t$E[keep_gene_a2, ]
...
sum(ge_sum$Expression_CV)
hist(ge_sum$Expression_CV)
ge_sum[abs(ge_sum$Expression_CV)>100,]
geneC_new[rownames(geneC_new)=="LOC111114812",]
```
...
final_pca <- final_pca[!is.na(final_pca$Methylation | final_pca$Methylation.CV),]
...
ggplot(final_pca,aes(y=Methylation,x=abs(Expression_CV)))+geom_point() + geom_smooth()
