...
**Quick comparison of different transformations of the gene count matrices**
```{r}
...
dapc_treatment <- dapc(t(early_time_counts_V1),early_time_meta$treatment,n.pca=2,n.da=1)
# PCs = 2
# clusters = 1
...
predict_values <- predict.dapc(dapc_treatment,t(late_time_counts_V1))
...
### Plotting data with PCA with isoform data
...
dapc_treatment <- dapc(t(early_time_counts),early_time_meta$treatment,n.pca=2,n.da=1)
# PCs = 2
# clusters = 1
...
plot(D9_400_Density, main="",xlim=c(-5,7),ylim=c(0,3),
     xlab="Discriminant Function 1")
polygon(D9_400_Density, col=alpha("lightblue",0.8), border="lightblue")
polygon(D9_2800_Density, col=alpha("tomato",0.8), border="red3")
polygon(D80_400_Density, col=alpha("blue",0.9), 
...
**Creating DF by treatment with first timepoint**  
```{r}
dapc_SFV_10<-dapc(t(tran$E),model$SFV,n.pca=8,n.da=3)
dapc_SFV_10<-dapc(t(tran$E),model$SFV)
# PCs = 10
# clusters = 3
output <- data.frame(Trt=model$Treatment,Time=model$Time,dapc_SFV_10$ind.coord)
