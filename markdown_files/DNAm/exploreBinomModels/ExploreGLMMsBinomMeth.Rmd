...
A lot of autocorrelation in the trace implies that this is not converging correctly!
...
* Order the levels so that 400_09 is the first level and recode the hypothesis tests accordingly. (Check that my coding of the hypothesis tests are correct first! - Need to understand "contrasts" better). Then go over re-coding together to make sure it is correct. Understand the contrasts to make sure hypothesis test is correct.
...
We therefore took a robust approach to analyzing metylation data with Bayesian Regression Models (BRMs) in which the sampling on the posterior distributions on the parameters was accomplished with the No-U-Turn-Sampler (NUTS). This algorithm converges much more quickly for high-dimensional models regardless of whether the priors are conjugate or not. (NEXT: describe model, brm(mc | trials(size)~ level + (1|tankID), data=a,
            family = binomial(), iter=5000, control = list(max_treedepth = 15, adapt_delta=0.99))
            (run without extra params to see the error messages they generate)
            higher adapt_delta avoids divergent transitions

* Write a function to test for main effects and an interaction, if the interaction is significant than also test for overlap of CI for pairwise params. For signifcant params store relevant outputs. Check that all output is there. Test on top 10 loci and 10 random loci to make sure everything works
