...
```{r cars}
packages_needed <- c("lme4", "multcomp", "devtools", "car", "MCMCglmm", "brms")
...
OK let's try the same thing, but with the data with 0's
```{r}
boxplot(mc/(mc+uc) ~ Treatment*Time, data=b, las=2)
# No random effect of tank on real data
...
# Add the random effect of tank
m3.1 <- brm(mc | trials(size)~ Treatment*Time + (1|tankID), data=b,family = binomial())
summary(m3.1)
plot(marginal_effects(m3.1), ask = FALSE)

# Try zero-inflated model
# https://paul-buerkner.github.io/brms/articles/brms_distreg.html
m3.2 <- brm(mc | trials(size)~ Treatment*Time + (1|tankID), data=b,family = zero_inflated_binomial())
summary(m3.2)
plot(marginal_effects(m3.2), ask = FALSE)
# The zero-inflated model doesn't seem to change the results at all in this case
```
...
# Add the random effect of tank
m3.1 <- brm(mc | trials(size)~ Treatment*Time + (1|tankID), data=c,family = binomial())
summary(m3.1)
plot(marginal_effects(m3.1), ask = FALSE)
  # get warnings for this data
...
# Try zero-inflated model
# https://paul-buerkner.github.io/brms/articles/brms_distreg.html
m3.2 <- brm(mc | trials(size)~ Treatment*Time + (1|tankID), data=c,family = zero_inflated_binomial())
summary(m3.2)
plot(m3.2, pars="Treatment")
plot(m3.2, pars="tankID")
