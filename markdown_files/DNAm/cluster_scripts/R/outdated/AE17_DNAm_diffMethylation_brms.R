...
INPUT_META <- "/home/downeyam/Github/2017OAExp_Oysters/input_files/meta"
...
### Data ####
mC <- readRDS(paste0(S_INPUT,"/Final_mC_gene_5.RData"))
uC <- readRDS(paste0(S_INPUT,"/Final_umC_gene_5.RData"))
meta_locus <- readRDS(paste0(S_INPUT,"/Final_meta_gene_5.RData"))
meta_samp <- readRDS("meta/metadata_20190811.RData")

# Range of samples (allows for quite subset to pseudo parallel code)
loci_range <- c(MINp:MAXp)

## Fix meta sample data
...
for(target in loci_range){
  tryCatch({
    ...
    png(paste0(OUTPUT,"/img/Locus_",meta_locus$cg_index[target],"_brmsEffectsPlot.png"))
    plot(m.out2)
    dev.off()
    # Save marginal effects
    png(paste0(OUTPUT,"/img/Locus_",meta_locus$cg_index[target],"_brmsMarginalEffectsPlot.png"))
    marginal_effects(m.out2,effects = "trt",plot=TRUE,ask=FALSE)
    marginal_effects(m.out2,effects = "trt:time",plot=TRUE,ask=FALSE)
    Sys.sleep(0.05)
    dev.off()
...
head(basic_comparisons$Evid.Ratio)
head(basic_comparisons$Significant)
sprintf("Saving outputs...")
saveRDS(model_param,paste0(OUTPUT,"/DNAm_gene_BRMS_",MINp,"_",MAXp,"_modelParam.RData"))
saveRDS(model_summary_stats,paste0(OUTPUT,"/DNAm_gene_BRMS_",MINp,"_",MAXp,"_modelSummary.RData"))
saveRDS(marginal_effects,paste0(OUTPUT,"/DNAm_gene_BRMS_",MINp,"_",MAXp,"_modelMarginalEffects.RData"))
saveRDS(basic_comparisons,paste0("DNAm_gene_BRMS_",MINp,"_",MAXp,"_plannedComparisons.RData"))
