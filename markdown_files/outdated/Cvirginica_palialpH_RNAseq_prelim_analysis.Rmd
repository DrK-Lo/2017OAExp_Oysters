...
```{r}
#setwd("~/Desktop/2017OAExp_Oysters/")
```

Read in the counts data table and the metadata table and view each
```{r}
setwd("~/Desktop/2017OAExp_Oysters/")
GeneCounts <- read.delim("results/C_virginica_gene_count_final.txt", header=TRUE, sep="", row.names=1)
model<-read.delim("input_files/metadata_cvirginica_rna_meta.txt", header=TRUE)
head(GeneCounts)
head(model)
```
