...
Determine proportion of counts explained by a single individual
```{r}
## Propotion of Reads due to a single individual  
# Determining the largest number of reads per gene for a single individual
GC_max <- apply(GeneCounts,1,max)
prop_single_ind_v2 <- GC_max/rowSums(GeneCounts) 
hist(prop_single_ind_v2)
...
count_data_df <- data.frame(name = row.names(GeneCounts),
                            Total_Reads =rowSums(GeneCounts),
                            Difference_Ratio = GC_diff,
                            Prop_Single_Ind = prop_single_ind,
                            Prop_with_Count = prop_with_count,
                            Gene_Count_Variance = Count_Variance)

ggplot(count_data_df,aes(x = Prop_with_Count,y = Gene_Count_Variance)) + geom_point() + geom_smooth(method="auto")

ggplot(count_data_df,aes(x = Prop_Single_Ind,y = Gene_Count_Variance)) + geom_point() + geom_smooth(method="auto")

ggplot(count_data_df,aes(x = Prop_Single_Ind,y = Prop_with_Count)) + geom_point() + geom_smooth(method="auto")
```
...
library(ggplot2)
ggplot(count_data_df,aes(x=Total_Reads,y=Difference_Ratio,colour=Prop_Single_Ind)) + geom_point() + 
...
ggplot(count_data_df,aes(x=Total_Reads,y=Difference_Ratio,colour=Gene_Count_Variance)) + geom_point() + 
  labs(title="Read count vs. Ratio of the difference in read count between lanes (per gene)",
       x="Total Reads (per Gene)",y="Lane Bias (0 is no bias, 1 biased to one lane)") +
  xlim(0,1000)
``` 
...
count_data_df$Difference_Trt <-  GC_trt_diff
...
count_data_df$Difference_Time <-  GC_time_diff

## Visualization
data_plot <- subset(count_data_df, count_data_df$Total_Reads < 100 & count_data_df$Total_Reads > 20)
...
GeneCounts[,model$treatment == 400]
